...
#
# Защита от повторного вызова на свежей базе: create_mb_schema уже не создаёт
# mb_id_tokens, поэтому на чистой цепочке миграций таблицы нет.
class DropMbIdTokens < ActiveRecord::Migration[7.0]
  def change
    if table_exists?(:mb_id_tokens)
